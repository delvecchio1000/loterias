#!/bin/zsh
##########################################################################
## conferir.sh - Shell script para loteria Quina			##
## Autor: Benedito Del Vecchio Junior                                   ##
## Data de criação: 01/12/2021                                          ##
## Descrição:                                                           ##
##		- lista os últimos concursos				##
...
end_site="https://www.mazusoft.com.br/quina/resultado.php?concurso=$n"
...
echo "Dezenas sorteadas $dezenas_concurso" >> log_jogos.txt

# descobrindo a quantidade de apostas para o concurso desejado
...
for a in $(seq 1 $quant_apostas) 
	...
	if [ $cont2 -eq 2 ]
		then	
		  ...
		  echo "${j1} ==> (PREMIADA_2)" >> log_jogos.txt
		  dois=$[$dois+1]
	elif [ $cont2 -eq 3 ]
		then	
		  echo "${j1} ==> $cont2 acertos (PREMIADA)"
		  echo "${j1} ==> (PREMIADA_3)" >> log_jogos.txt
		  tres=$[$tres+1]
	elif [ $cont2 -eq 4 ]
		then	
		  echo "${j1} ==> $cont2 acertos (PREMIADA)"
...
echo " " >> log_jogos.txt
echo "2 acertos: $dois" >> log_jogos.txt
echo "3 acertos: $tres" >> log_jogos.txt
